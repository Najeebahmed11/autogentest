...
def delete_model(model: Model, dbmanager: DBManager) -> List[dict]:
    ...
    query = "DELETE FROM models WHERE id = ? OR user_id = ?"
...
def create_message(message: Message, dbmanager: DBManager) -> List[dict]:
    ...
    root_messages = get_messages_root(root_msg_id=message.root_msg_id,user_id=message.user_id, session_id=message.session_id, dbmanager=dbmanager)
    return root_messages



def get_messages_root(root_msg_id: str,user_id: str, session_id: str, dbmanager: DBManager) -> List[dict]:
    """
    ...
    """
    query = "SELECT * FROM messages WHERE root_msg_id = ? user_id = ? AND session_id = ?"
    args = (root_msg_id,user_id, session_id)
